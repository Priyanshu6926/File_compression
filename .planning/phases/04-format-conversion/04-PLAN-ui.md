...
phase: 4
wave: 2
title: "Target Format UI Controls"
depends_on: ["4.1"]
requirements_addressed: [CONV-01, CONV-02]
files_modified:
  - src/components/controls/ControlsPanel.tsx
autonomous: true
---

# Plan 4.2 — Target Format UI Controls

## Objective

Build the UI for selecting the output format (JPEG, PNG, WEBP, AVIF, TIFF) as per the Neo-Brutalist design specs, and wire it up to the `FormData` submission.

## Tasks

<task id="4.2.1">
<title>Update ControlsPanel with format selector</title>
<read_first>
- src/components/controls/ControlsPanel.tsx
- .planning/phases/01-project-scaffolding-upload/01-UI-SPEC.md (Section 6.4)
</read_first>
<action>
Update `src/components/controls/ControlsPanel.tsx` to include "TARGET FORMAT" grid:
...
<acceptance_criteria>
- `src/components/controls/ControlsPanel.tsx` includes "TARGET FORMAT" selection buttons
- The format buttons match the Neo-Brutalist tags styling (popping up with shadows on active state)
- State variable `format` correctly binds to the selection and is sent in `FormData`
- The `result` state captures the format to generate correct `download` file extensions
...
```
must_haves:
  - "TARGET FORMAT" grid is rendered and interactive
  - Selected format state persists and visually clearly shows selection
  - The format is submitted via FormData
  - Download filename respects the chosen format
